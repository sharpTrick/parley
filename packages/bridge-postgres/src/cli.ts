...
async function main(): Promise<void> {
  ...
  if (args.kind === 'error') {
    process.stderr.write(`parley-postgres: ${args.message}\n${USAGE}\n`);
    process.exit(2);
  }
  ...
  process.stderr.write(
    `parley-postgres: bridge up — handle=${cfg.identity.handle} topics=[${cfg.topics.join(', ')}] ` +
...
main().catch((err: unknown) => {
  ...
  process.stderr.write(`parley-postgres: fatal: ${detail}\n`);
  process.exit(1);
});
