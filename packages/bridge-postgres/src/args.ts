...
export const USAGE = `usage: parley-postgres [--config <path>] [--help] [--version]

  -c, --config <path>  config file (default: $PARLEY_CONFIG, else parley.config.yaml)
  -h, --help           print this message
  -V, --version        print the package version`;

export const POSTGRES_VERSION: string = (
 ...
 * the default names a different deployment: another database url, table, handle and topic
...
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--version' || arg === '-V') return { kind: 'print', text: POSTGRES_VERSION };
    if (arg === '--config' || arg === '-c') {
      const next = argv[i + 1];
