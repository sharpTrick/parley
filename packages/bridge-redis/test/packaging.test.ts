...
// CLASS: a promise this package's manifest makes that only CONFIGURATION keeps, so nothing fails
// when the configuration is simply absent. Its ~3,900 lines of test source were inside no tsconfig
// while CI's typecheck step justifies itself by claiming the opposite (`--if-present` exits 0 for a
// package with no `typecheck:test` script, which is a silent skip, not a check); and `files:
// ["dist"]` with no LICENSE beside package.json publishes a tarball that declares "license": "MIT"
// and ships no licence text, because npm packs LICENSE only from the package directory.
//
// Every rule below is derived from the manifest, from ci.yml and from what is on disk — never from
// a list kept by hand, which is the thing that went missing in the first place.
...
function packed(path: string): boolean {
  ...
  return (manifest.files ?? []).some(
    (f) =>
      p === f.replace(/^\.\//, '') ||
      p.startsWith(`${f.replace(/^\.\//, '').replace(/\/$/, '')}/`),
  );
}
...
describe('the published tarball carries everything the manifest promises', () => {
  ...
  it.each(PROMISED)('%s is inside the "files" list', (path) => {
    expect(
      packed(path),
      `add a "files" entry covering ${path}, or the tarball cannot resolve it`,
    ).toBe(true);
  });
