...
export const USAGE = `usage: parley-redis [--config <path>] [--help] [--version]

  -c, --config <path>  config file (default: $PARLEY_CONFIG, else parley.config.yaml)
  -h, --help           print this message
  -V, --version        print the package version`;

export const REDIS_VERSION: string = (
 ...
 * the default names a different deployment: another url, key_prefix, handle and topic allowlist,
 * and the only trace is a stderr line an MCP stdio host routinely discards.
 */
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--version' || arg === '-V') return { kind: 'print', text: REDIS_VERSION };
    if (arg === '--config' || arg === '-c') {
      const next = argv[i + 1];
