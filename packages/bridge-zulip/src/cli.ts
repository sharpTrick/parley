...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `parley-zulip: bridge up — handle=${cfg.identity.handle} topics=[${cfg.topics.join(', ')}] ` +
  ...
  // An orphaned bridge (parent crashed or SIGKILLed) gets stdin EOF and no signal — keep these,
...
main().catch((err: unknown) => {
  ...
  process.stderr.write(`parley-zulip: fatal: ${detail}\n`);
  process.exit(1);
});
