...
parley-nats --config parley.config.yaml
# or: node packages/bridge-nats/dist/cli.js --config parley.config.yaml
# or: PARLEY_CONFIG=parley.config.yaml parley-nats
parley-nats --help      # also --version
```
...
back to the default `parley.config.yaml`, since that default names a different deployment's another cluster, stream prefix, handle and topic allowlist.
