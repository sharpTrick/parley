...
import { type ParsedArgs, parseArgs, SLACK_VERSION, USAGE } from '../src/args.js';

// CLASS: an argv the CLI cannot honour must stop it. The default config names a whole other
// deployment — another workspace's `bot_token` and `app_token`, its own `channel_map`, handle and
// topic allowlist — so falling back to it because an argument was mistyped, or because the shell
// ate `--config`'s value, brings a bridge up as the wrong bot in the wrong workspace and posts
// agent output into channels the operator never selected, with nothing but a stderr line an MCP
// stdio host routinely discards to say so.
//
...
beforeAll(() => {
  ...
  execFileSync('npx', ['tsc', '-b', 'packages/bridge-slack', '--force'], {
    cwd: REPO,
    stdio: 'pipe',
  });
...
const HONOURED: Array<[argv: string[], parsed: ParsedArgs]> = [
  ...
  [['--version'], { kind: 'print', text: SLACK_VERSION }],
  [['-V'], { kind: 'print', text: SLACK_VERSION }],
];
