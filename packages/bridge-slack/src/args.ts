...
export const USAGE = `usage: parley-slack [--config <path>] [--help] [--version]

  -c, --config <path>  config file (default: $PARLEY_CONFIG, else parley.config.yaml)
  -h, --help           print this message
  -V, --version        print the package version`;

export const SLACK_VERSION: string = (
 ...
 * the default names a different deployment: another workspace's `bot_token` and `app_token`, its
 * own `channel_map`, handle and topic allowlist, and the only trace is a stderr line an MCP stdio
 * host routinely discards.
 */
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--version' || arg === '-V') return { kind: 'print', text: SLACK_VERSION };
    if (arg === '--config' || arg === '-c') {
      const next = argv[i + 1];
