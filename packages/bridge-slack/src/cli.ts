...
// to stderr. Only a branch that exits BEFORE the transport exists may write to stdout.
...
async function main(): Promise<void> {
  ...
  if (args.kind === 'error') {
    process.stderr.write(`parley-slack: ${args.message}\n${USAGE}\n`);
    process.exit(2);
  }
  ...
  process.stderr.write(
    `parley-slack: bridge up — handle=${cfg.identity.handle} topics=[${cfg.topics.join(', ')}] ` +
...
main().catch((err: unknown) => {
  ...
  process.stderr.write(`parley-slack: fatal: ${detail}\n`);
  process.exit(1);
});
