...
export const USAGE = `usage: parley-xmpp [--config <path>] [--help] [--version]

  -c, --config <path>  config file (default: $PARLEY_CONFIG, else parley.config.yaml)
  -h, --help           print this message
  -V, --version        print the package version`;

export const XMPP_VERSION: string = (
 ...
 * the default names a different deployment: another service, muc_service, identity.handle and topic
...
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--version' || arg === '-V') return { kind: 'print', text: XMPP_VERSION };
    if (arg === '--config' || arg === '-c') {
      const next = argv[i + 1];
