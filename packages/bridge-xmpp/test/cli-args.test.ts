...
import { type ParsedArgs, parseArgs, USAGE, XMPP_VERSION } from '../src/args.js';

// CLASS: an argv the CLI cannot honour must stop it. The default config names a whole other
// deployment — its own service, muc_service, identity.handle and topic allowlist — so falling back
// to it because an argument was mistyped, or because the shell ate `--config`'s value, starts a
// bridge against the wrong rooms and posts agent output into topics the operator never selected,
...
beforeAll(() => {
  ...
  execFileSync('npx', ['tsc', '-b', 'packages/bridge-xmpp', '--force'], {
    cwd: REPO,
    stdio: 'pipe',
  });
...
const HONOURED: Array<[argv: string[], parsed: ParsedArgs]> = [
  ...
  [['--version'], { kind: 'print', text: XMPP_VERSION }],
  [['-V'], { kind: 'print', text: XMPP_VERSION }],
];
